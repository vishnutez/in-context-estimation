...
from tasks import (
    get_task_sampler,
    PolynomialsUnbiasedPoints,
    PolynomialsDegTwoMonomialSelectionUnbiased,
    LinearRegressionRegVecEst,
)
...
def eval_batch(
    ...
    task = task_sampler()
    if torch.cuda.is_available() and model.name.split("_")[0] in ["gpt2", "lstm", "bert"]:
        device = "cuda"
    else:
        device = "cpu"
    ...
    if xs_p is None:
        if isinstance(task, PolynomialsUnbiasedPoints):
            ...
            ys, _ = task.rejection_sample_to_form_batch(
                ...
                bsize=bsize,
                n_points=n_points,
                excess_tensors=excess_tensors_eval,
            )
        ...
        if isinstance(model, models.InverseProblemBidirectionalTransformerModel):
            pred = bert_eval_with_masking(model, xs.to(device), ys.to(device)).detach()
        else:
            pred = model(xs.to(device), ys.to(device)).detach()

        bsize, n_points, n_dims = xs.shape

        if isinstance(model, models.InverseProblemTransformerModel) or isinstance(model, models.InverseProblemBidirectionalTransformerModel):
            # Swap xs and ys for the purpose of loss computation.
            # TODO: better long-term solution
            metrics = task.get_metric()(pred.cpu(), xs)
        elif isinstance(task, LinearRegressionRegVecEst):
            metrics = task.get_metric()(pred.cpu(), torch.tile(torch.transpose(ws, -1,-2), (1, n_points, 1)))
        else:
        ...
        for i in range(n_points):
            ...
            if isinstance(model, models.InverseProblemBidirectionalTransformerModel):
                raise NotImplementedError("This evaluation method is not implemented for the BERT model.")
                pred = xs_comb  # TODO this is a placeholder, need to get this working with masking
            else:
                pred = model(xs_comb.to(device), ys.to(device), inds=[i]).detach()
            if isinstance(model, models.InverseProblemTransformerModel) or isinstance(model, models.InverseProblemBidirectionalTransformerModel):
                # Swap xs and ys for the purpose of loss computation.
                # TODO: better long-term solution
                metrics[:, i] = task.get_metric()(pred.cpu(), xs_comb)[:, i]
            else:
...
def eval_model(
    ...
    for i in range(num_eval_examples // batch_size):  # every iteration creates a batch
...
def conf_to_model_name(conf):
    if conf.model.family in ["gpt2", "gpt2_inverse_problem"]:
        return {
            (3, 2): "Transformer-xs",
