...
def get_prompts_from_task(eval_prompts_task):
    ...
    if (
        eval_prompts_task == "relu_2nn_regression"
        or eval_prompts_task == "relu_3nn_regression"
    ):
...
def get_gradient_descent_reference_preds(
    xs, ys, args, train_steps=5000
):  # xs --> 64, 101, 20; ys --> 64, 101
    nn_model = 0
    ...
    # gd_model = GDModel(nn_model, {'in_size': args["n_dims"], 'hidden_size': args["hidden_size"], 'out_size' :1}, opt_alg = 'adam', batch_size = 10, lr = args["learning_rate"], num_steps = 5000)
    gd_model = GDModel(
        nn_model,
        {"in_size": args["n_dims"], "hidden_size": args["hidden_size"], "out_size": 1},
        opt_alg="adam",
        batch_size=10,
...
def get_gradient_descent_preds_and_reference_errors(
    xs, ys, args, train_steps=5000
):  # xs --> 64, 101, 20; ys --> 64, 101
    gd_preds = (
        get_gradient_descent_reference_preds(xs, ys, args, train_steps=train_steps)
        .detach()
        .cpu()
    )  # 64, 101
    return (
        gd_preds[:, 1:],
        (ys[:, 1:] - gd_preds[:, 1:]).square().mean(axis=0).numpy(),
    )  # [64, 100], [100,]
...
def plot_prediction_diff_wrt_ground_truth(
    transformer_loss: list, task_losses: dict, task_kwargs: dict = {}
):
    ...
    if "sign_vec_cs" in task_losses:
        ...
        plt.plot(
            x_axis_items,
            pred_diff_wrt_GT,
            label=get_plot_label_MSE_wrt_GT("Inf Norm Minimization"),
        )
        plt.scatter(
            task_kwargs["sign_vec_cs"]["bound"] + 1, 0, color="red", label="Bound"
        )
    if "linear_regression" in task_losses:
        ...
        plt.plot(
            ...
            pred_diff_wrt_GT,
            lw=2,
            label=get_plot_label_MSE_wrt_GT("Least Squares"),
        )
    if "sparse_regression" in task_losses:
        ...
        plt.plot(
            ...
            pred_diff_wrt_GT,
            lw=2,
            label=get_plot_label_MSE_wrt_GT("Lasso"),
        )
    if "relu_2nn_regression" in task_losses:
        ...
        plt.plot(
            ...
            pred_diff_wrt_GT,
            lw=2,
            label=get_plot_label_MSE_wrt_GT("2-layer NN (GD)"),
        )
    if "relu_3nn_regression" in task_losses:
        ...
        plt.plot(
            ...
            pred_diff_wrt_GT,
            lw=2,
            label=get_plot_label_MSE_wrt_GT("3-layer NN (GD)"),
        )
...
def plot_prediction_diff(
    ...
    task_preds: dict,  # task_preds["task name"] == b, n_points-1
    task_kwargs: dict = {},
):
    ...
    if "sign_vec_cs" in task_preds:
        ...
        plt.plot(
            x_axis_items, pred_diff, label="MSE(TF_preds, Inf Norm Minimization preds"
        )
        plt.scatter(
            task_kwargs["sign_vec_cs"]["bound"] + 1, 0, color="red", label="Bound"
        )
    if "linear_regression" in task_preds:
        ...
        plt.plot(
            x_axis_items, pred_diff, lw=2, label="MSE(TF_preds, Least Squares preds)"
        )
    ...
    if "relu_2nn_regression" in task_preds:
        ...
        plt.plot(
            x_axis_items, pred_diff, lw=2, label="MSE(TF_preds, 2-layer NN (GD) preds)"
        )
    if "relu_3nn_regression" in task_preds:
        pred_diff = getMSE(transformer_preds, task_preds["relu_3nn_regression"])
        plt.plot(
            x_axis_items, pred_diff, lw=2, label="MSE(TF_preds, 3-layer NN (GD) preds)"
        )
...
def plot_results_with_sd_bars(
    ...
    # all the loss array passed to this method must be of shape batch_size x n_points-1
    fig = plt.figure()
    ax = plt.gca()

    sns.lineplot(
        data=get_df_from_pred_array(transformer_loss),
        y="y",
        x="x",
        ci="sd",
        lw=2,
    ...
    if "sign_vec_cs" in task_losses:
        sns.lineplot(
            data=get_df_from_pred_array(task_losses["sign_vec_cs"]),
            y="y",
            x="x",
            ci="sd",
            lw=2,
            label="Inf Norm Minimization",
            ax=ax,
        )
        plt.scatter(
            task_kwargs["sign_vec_cs"]["bound"] + 1,
            0,
            color="red",
            label="Bound",
            ax=ax,
    ...
    if "linear_regression" in task_losses:
        sns.lineplot(
            data=get_df_from_pred_array(task_losses["linear_regression"]),
            y="y",
            x="x",
            ci="sd",
            lw=2,
    ...
    if "sparse_regression" in task_losses:
        sns.lineplot(
            data=get_df_from_pred_array(task_losses["sparse_regression"]),
            y="y",
            x="x",
            ci="sd",
            lw=2,
    ...
    if "relu_2nn_regression" in task_losses:
        sns.lineplot(
            data=get_df_from_pred_array(task_losses["relu_2nn_regression"]),
            y="y",
            x="x",
            ci="sd",
            lw=2,
    ...
    if "relu_3nn_regression" in task_losses:
        sns.lineplot(
            data=get_df_from_pred_array(task_losses["relu_3nn_regression"]),
            y="y",
            x="x",
            ci="sd",
            lw=2,
...
def plot_results(
    ...
    **kwargs,
    ...
    # x_axis_items = np.arange(1, transformer_loss.shape[0]+1)
    plot_from_index_x_axis = (
        kwargs["plot_from_index_x_axis"] if "plot_from_index_x_axis" in kwargs else 1
    )
    ...
    if "sign_vec_cs" in task_losses:
        plt.plot(
            x_axis_items,
            task_losses["sign_vec_cs"][:points_to_plot],
            label="Inf Norm Minimization",
        )
        plt.scatter(
            task_kwargs["sign_vec_cs"]["bound"] + 1, 0, color="red", label="Bound"
    ...
    if "linear_regression" in task_losses:
        plt.plot(
            x_axis_items,
            task_losses["linear_regression"][:points_to_plot],
            lw=2,
            label="Least Squares",
        )
    if "sparse_regression" in task_losses:
        plt.plot(
            x_axis_items,
            task_losses["sparse_regression"][:points_to_plot],
            lw=2,
            label="Lasso",
        )
    if "relu_2nn_regression" in task_losses:
        plt.plot(
            x_axis_items,
            task_losses["relu_2nn_regression"][:points_to_plot],
            lw=2,
            label="2-layer NN, GD",
        )
    if "relu_3nn_regression" in task_losses:
        plt.plot(
            x_axis_items,
            task_losses["relu_3nn_regression"][:points_to_plot],
            lw=2,
            label="3-layer NN, GD",
        )
    if "PME T1" in task_losses:
        plt.plot(
            x_axis_items, task_losses["PME T1"][:points_to_plot], lw=2, label="PME T1"
        )
    if "PME T2" in task_losses:
        plt.plot(
            x_axis_items, task_losses["PME T2"][:points_to_plot], lw=2, label="PME T2"
        )
    if "PME Mix." in task_losses:
        plt.plot(
            x_axis_items,
            task_losses["PME Mix."][:points_to_plot],
            lw=2,
            label="PME Mix.",
        )
...
def compute_average_loss_difference(loss1, loss2, printString):
    ...
    print(
        f"Average difference between losses {printString}:",
        np.abs(loss1 - loss2).mean(),
    )
...
def load_GMM_model_with_run_id(run_id):
    ...
    #     run_id = "327f469c-1601-4a74-a649-b6ef8b87612b"  # if you train more models, replace with the run_id from the table above
    ...
    return model, conf, n_dims, conf.training.task_kwargs["mixing_ratio"]
...
def sample_xy(n_points, batch_size, conf, n_dims):
    ...
    ys_t1, ys_t2, task_t1, task_t2 = None, None, None, None
    ...
    for prompts_from_task in [0, 1]:
        ...
        task_sampler = get_task_sampler(
            conf.training.task, n_dims, batch_size, **task_kwargs_copy
        )
        task = task_sampler()
    ...
    # sample xs, ys
    return xs, ys_t1, ys_t2, task_t1, task_t2
...
def get_TF_and_LSQ(model, xs, ys, task):
    ...
    # args={
    #         "nn_layer_num": get_nn_layer_from_task(eval_prompts_task),
    #         "n_dims": n_dims,
    #         "hidden_size": task_kwargs.hidden_layer_size,
    #         "learning_rate": get_learning_rate_for_GD(get_prompts_from_task(eval_prompts_task))
    #     }
    # gd_preds, gradient_descent_errors = get_gradient_descent_preds_and_reference_errors(xs, ys, args, train_steps=100)
    lsq_errors, lsq_preds = get_linear_regression_rerefence_errors_no_mean(xs, ys)
    # lasso_errors = get_sparse_regression_reference_errors(xs, ys)
    # baseline_errors_batch = get_sign_vec_cs_reference_errors(xs, ys, n_dims)
    # print("Number of inf value in CS solution i.e, number of infeasible problems", np.sum(np.isinf(baseline_errors_batch)))
...
def plot_all(
    ...
    pme_t1,
    t1_errors_batch_no_mean,
    t1_preds_batch_no_mean,
    pme_t2,
    t2_errors_batch_no_mean,
    t2_preds_batch_no_mean,
    pme_mix,
    pmix_errors_batch_no_mean,
    pmix_preds_batch_no_mean,
    points_to_plot,
    plot_title,
    no_LSQ=False,
    to_plot=["errorbar", "usual-lineplot", "MSE_diff_wrt_GT", "MSE_diff_direct"],
    **kwargs,
):
    ...
    # w_b = task.w_b
    # w_b, w_probed, error, cos_sim = recover_weights(model, xs, ys, w_b)

    # w_probed_prompt_len.append(w_probed[:,0])


def sns_plot_heatmap(
    w_plot,
    figsize,
    cmap,
    text_dict,
    ...
    if is_numpy:
        sns.heatmap(
            w_plot.round(round_to_digits).transpose(),
            linewidth=0.3,
            annot=annot,
            annot_kws={"size": 4},
            cmap=cmap,
        ...
        sns.heatmap(
            w_plot.numpy().round(round_to_digits).transpose(),
            linewidth=0.3,
            annot=annot,
            annot_kws={"size": 4},
            cmap=cmap,
