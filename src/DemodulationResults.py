...
if not args.load_predictions:
    ...
    with torch.no_grad():
        ...
        mix_transformer_ray_preds = demod_model(xs.to(cuda_device), ray_ys.to(cuda_device)).cpu()
        print('Getting Fading Preds for Mixture Model')
        mix_transformer_fading_preds = demod_model(xs.to(cuda_device), fading_ys.to(cuda_device)).cpu()
    ...
    if args.save_predictions:
        ...
        np.save(ray_errors_file, mix_transformer_ray_errors, allow_pickle=False)
        np.save(fading_errors_file, mix_transformer_fading_errors, allow_pickle=False)
else:
...
plt.savefig(f"plots/{args.task_name}_{numlines}lines.png", dpi=300, bbox_inches="tight")
